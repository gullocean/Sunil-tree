...
var openFileNames = ["data.json", "test.json"];
var saveFileName = "";
...
//click save button
function save() {
  saveData(jsonData, saveFileName + ".json");
...
function openFile() {
  ...
  d3.json("json/" + openFileName, function(error, data) {
    d3.select("svg").remove();
    updateTree(data);
  });
}
...
function updateTree(treeData) {
  ...
  function update(source) {
    ...
    nodeEnter
      ...
      .on('contextmenu', d3.contextMenu(context_menu_node));
    ...
    nodeEnter.append("circle").attr('class', 'ghostCircle').attr("r", ghostR).attr("opacity", 0.2) // change this to zero to hide the target area
    ...
    node.select("circle.nodeCircle").attr("r", nodeR).style("fill", function(d) {
      return d._children ? "lightsteelblue" : "#fff";
    });
