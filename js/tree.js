...
function visit(parent, visitFn, childrenFn) {
  ...
  if (children) {
    ...
    for (var ii = 0; ii < count; ii++) {
      visit(children[ii], visitFn, childrenFn);
    }
  }
}
...
var margin = {
  ...
  viewerWidth = $(document).width() - margin.right - margin.left,
...
var zoomListener = d3.behavior.zoom().scaleExtent([1, 13]).on("zoom", zoom);
...
function updateTree(source) {
  ...
  nodeEnter
    ...
    .attr("r", ghostR).attr("opacity", 0.2) // change this to zero to hide the target area
    .style("fill", "red")
